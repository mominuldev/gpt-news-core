...
function gpt_sync_mailchimp( $MailchimpAPI, $MailchimpLID, $data ) {
 ...
 */
function gpt_mailchimp_subscribe() {

	// API and List ID
	$APIKey 	= get_theme_mod( 'gpt_mailchimp_api_key' );
	$ListID 	= get_theme_mod( 'gpt_mailchimp_api_list' );
	...
	if ( ! empty( $APIKey ) && ! empty( $ListID ) ) {
		...
		if ( ! empty( $_REQUEST['email'] ) ) {
			...
			$makeRequest = gpt_sync_mailchimp( $APIKey, $ListID, $data );
...
add_action( 'wp_ajax_nopriv_gpt_mailchimp_subscribe', function() {

	header('Content-type: application/json');
	echo json_encode( gpt_mailchimp_subscribe() );
	die();

});
add_action( 'wp_ajax_gpt_mailchimp_subscribe', function() {
