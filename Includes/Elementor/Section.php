...
class Section {
	...
	function post_metaboxes( $page ) {
		...
		$page->start_controls_section( 'header_options', [
			'label' => esc_html__( 'MPT Header Options', 'gpt-core' ),
			'tab'   => Controls_Manager::TAB_SETTINGS,
		] );

		$page->add_control( 'mobile_breakpoint', [
			'label'   => esc_html__( 'Mobile Header resolution breakpoint', 'gpt-core' ),
		...
		$page->add_control( 'header_on_bg', [
			'label'        => esc_html__( 'Over content?', 'gpt-core' ),
			'description'  => esc_html__( 'Set Header to display over content.', 'gpt-core' ),
	...
	public function extened_row_render( \Elementor\Element_Base $element ) {
		...
		if ( isset( $settings['add_background_text'] ) && ! empty( $settings['add_background_text'] ) ) {
			...
			wp_enqueue_script( 'appear', esc_url( MPT_SCRIPTS . '/jquery.appear.js' ), [], false, false );
			wp_enqueue_script( 'anime', esc_url( MPT_SCRIPTS . '/anime.min.js' ), [], false, false );
		}
		...
		if ( isset( $settings['add_background_animation'] ) && ! empty( $settings['add_background_animation'] ) ) {
			if ( ! (bool) Plugin::$instance->editor->is_edit_mode() ) {
				wp_enqueue_script( 'parallax', esc_url( MPT_SCRIPTS . '/parallax.min.js' ), [], false, false );
				wp_enqueue_script( 'paroller', esc_url( MPT_SCRIPTS . '/jquery.paroller.min.js' ), [], false, false );
				//wp_enqueue_style( 'animate', esc_url( MPT_SCRIPTS . 'assets/css/animate.css' ) );
			}
		}
	...
	public function extened_column_render( \Elementor\Element_Base $element ) {
		...
		if ( isset( $settings['apply_sticky_column'] ) && ! empty( $settings['apply_sticky_column'] ) ) {
			wp_enqueue_script( 'theia-sticky-sidebar', MPT_SCRIPTS . '/theia-sticky-sidebar.min.js', [], false, false );
		}
	}
	...
	public function enqueue_scripts() {
		...
		if ( (bool) Plugin::$instance->preview->is_preview_mode() ) {
			//wp_enqueue_style( 'animate', esc_url( MPT_SCRIPTS . '/css/animate.css' ) );
		...
		wp_enqueue_script( 'gpt-parallax', esc_url( MPT_SCRIPTS . '/elementor_sections.js' ), [ 'jquery' ], false, true );

		//Add options in the column
		wp_enqueue_script( 'gpt-column', esc_url( MPT_SCRIPTS . '/elementor_column.js' ), [ 'jquery' ], false, true );

		wp_localize_script( 'gpt-parallax', 'gpt_parallax_settings', [
			$this->sections,
			'ajaxurl' => admin_url( 'admin-ajax.php' ),
			'svgURL'  => esc_url( MPT_CORE_ASSETS_URL . 'shapes/' ),
		] );
	}

	public function extened_animation( $widget, $args ) {
		$widget->start_controls_section( 'extened_animation', [
			'label' => esc_html__( 'MPT Background Text', 'gpt-core' ),
			'tab'   => Controls_Manager::TAB_STYLE,
		] );

		$widget->add_control( 'add_background_text', [
			'label'        => esc_html__( 'Add Background Text?', 'gpt-core' ),
			'type'         => Controls_Manager::SWITCHER,
			'label_on'     => esc_html__( 'On', 'gpt-core' ),
			'label_off'    => esc_html__( 'Off', 'gpt-core' ),
		...
		$widget->add_control( 'background_text', [
			'label'       => esc_html__( 'Background Text', 'gpt-core' ),
			'type'        => Controls_Manager::TEXTAREA,
			'label_block' => true,
			'default'     => esc_html__( 'Text', 'gpt-core' ),
			'selectors'   => [
		...
		$widget->add_responsive_control( 'background_text_indent', [
			'label'      => esc_html__( 'Text Indent', 'gpt-core' ),
		...
		$widget->add_responsive_control( 'background_text_spacing', [
			'label'     => esc_html__( 'Top Spacing', 'gpt-core' ),
			'type'      => Controls_Manager::SLIDER,
			'selectors' => [
		...
		$widget->add_control( 'apply_animation_background_text', [
			'label'        => esc_html__( 'Apply Animation?', 'gpt-core' ),
			'type'         => Controls_Manager::SWITCHER,
			'label_on'     => esc_html__( 'On', 'gpt-core' ),
			'label_off'    => esc_html__( 'Off', 'gpt-core' ),
		...
		$widget->start_controls_section( 'extened_parallax', [
			'label' => esc_html__( 'MPT Parallax', 'gpt-core' ),
			'tab'   => Controls_Manager::TAB_STYLE,
		] );

		$widget->add_control( 'add_background_animation', [
			'label'        => esc_html__( 'Add Extended Background Animation?', 'gpt-core' ),
			'type'         => Controls_Manager::SWITCHER,
			'label_on'     => esc_html__( 'On', 'gpt-core' ),
			'label_off'    => esc_html__( 'Off', 'gpt-core' ),
			'return_value' => 'yes',
		...
		$repeater->add_control( 'image_effect', [
			'label'   => esc_html__( 'Parallax Effect', 'gpt-core' ),
			'type'    => Controls_Manager::SELECT,
			'options' => [
				'scroll'        => esc_html__( 'Scroll', 'gpt-core' ),
				'mouse'         => esc_html__( 'Mouse', 'gpt-core' ),
				'css_animation' => esc_html__( 'CSS Animation', 'gpt-core' ),
			],
			'default' => 'scroll',
		] );

		$repeater->add_responsive_control( 'animation_name', [
			'label'     => esc_html__( 'Animation', 'gpt-core' ),
		...
		$repeater->add_control( 'animation_name_iteration_count', [
			'label'     => esc_html__( 'Animation Iteration Count', 'gpt-core' ),
			'type'      => Controls_Manager::SELECT,
			'default'   => '1',
			'options'   => [
				'infinite' => esc_html__( 'Infinite', 'gpt-core' ),
		...
		$repeater->add_control( 'animation_name_speed', [
			'label'     => esc_html__( 'Animation speed', 'gpt-core' ),
			'type'      => Controls_Manager::NUMBER,
			'min'       => 1,
			'step'      => 100,
			'default'   => '1',
		...
		$repeater->add_control( 'animation_name_direction', [
			'label'     => esc_html__( 'Animation Direction', 'gpt-core' ),
			'type'      => Controls_Manager::SELECT,
			'default'   => 'normal',
			'options'   => [
				'normal'    => esc_html__( 'Normal', 'gpt-core' ),
				'reverse'   => esc_html__( 'Reverse', 'gpt-core' ),
				'alternate' => esc_html__( 'Alternate', 'gpt-core' ),
			],
			'condition' => [
				'image_effect' => 'css_animation',
			],
		...
		$repeater->add_control( 'image_bg', [
			'label'       => esc_html__( 'Parallax Image', 'gpt-core' ),
		...
		$repeater->add_control( 'parallax_dir', [
			'label'     => esc_html__( 'Parallax Direction', 'gpt-core' ),
			'type'      => Controls_Manager::SELECT,
			'options'   => [
				'vertical'   => esc_html__( 'Vertical', 'gpt-core' ),
				'horizontal' => esc_html__( 'Horizontal', 'gpt-core' ),
			],
			'condition' => [
				'image_effect' => 'scroll',
			],
		...
		$repeater->add_control( 'parallax_factor', [
			'label'       => esc_html__( 'Parallax Factor', 'gpt-core' ),
			'type'        => Controls_Manager::NUMBER,
			'min'         => - 3,
			'max'         => 3,
			'step'        => 0.01,
			'default'     => 0.03,
			'description' => esc_html__( 'Set elements offset and speed. It can be positive (0.3) or negative (-0.3). Less means slower.', 'gpt-core' ),
		] );

		$repeater->add_responsive_control( 'position_top', [
			'label'       => esc_html__( 'Top Offset', 'gpt-core' ),
			'type'        => Controls_Manager::SLIDER,
			'size_units'  => [ '%', 'px' ],
			'range'       => [
				'%'  => [ 'min' => - 100, 'max' => 100 ],
			...
			'default'     => [ 'size' => 0, 'unit' => '%' ],
			'description' => esc_html__( 'Set figure vertical offset from top border.', 'gpt-core' ),
			'selectors'   => [
		...
		$repeater->add_responsive_control( 'position_left', [
			'label'       => esc_html__( 'Left Offset', 'gpt-core' ),
			'type'        => Controls_Manager::SLIDER,
			'size_units'  => [ '%', 'px' ],
			'range'       => [
				'%'  => [
			...
			'description' => esc_html__( 'Set figure horizontal offset from left border.', 'gpt-core' ),
			'selectors'   => [
		...
		$repeater->add_control( 'image_index', [
			'label'     => esc_html__( 'Image z-index', 'gpt-core' ),
			'type'      => Controls_Manager::NUMBER,
			'step'      => 1,
			'default'   => - 1,
			'selectors' => [
		...
		$repeater->add_control( 'hide_on_mobile', [
			'label'        => esc_html__( 'Hide On Mobile?', 'gpt-core' ),
			'type'         => Controls_Manager::SWITCHER,
			'label_on'     => esc_html__( 'On', 'gpt-core' ),
			'label_off'    => esc_html__( 'Off', 'gpt-core' ),
			'return_value' => 'yes',
		...
		$repeater->add_control( 'hide_mobile_resolution', [
			'label'     => esc_html__( 'Screen Resolution', 'gpt-core' ),
			'type'      => Controls_Manager::NUMBER,
			'step'      => 1,
			'default'   => 768,
			'condition' => [
		...
		$widget->add_control( 'items_parallax', [
			'label'     => esc_html__( 'Layers', 'gpt-core' ),
		...
		$widget->start_controls_section( 'extened_shape', [
			'label' => esc_html__( 'MPT Shape Divider', 'gpt-core' ),
			'tab'   => Controls_Manager::TAB_STYLE,
		] );
		...
		$shapes_options = [
			...
			'torn_line' => esc_html__( 'Torn Line', 'gpt-core' ),
		];

		foreach (
			[
				'top'    => esc_html__( 'Top', 'gpt-core' ),
				'bottom' => esc_html__( 'Bottom', 'gpt-core' ),
			] as $side => $side_label
		) {
			...
			$widget->add_control( $base_control_key, [
				'label'   => esc_html__( 'Type', 'gpt-core' ),
				'type'    => Controls_Manager::SELECT,
			...
			$widget->add_responsive_control( $base_control_key . '_height', [
				'label'     => esc_html__( 'Height', 'gpt-core' ),
				'type'      => Controls_Manager::SLIDER,
				'range'     => [
					'px' => [
						'max' => 500,
			...
			$widget->add_control( $base_control_key . '_above_content', [
				'label'     => esc_html__( 'Z-index', 'gpt-core' ),
				'type'      => Controls_Manager::NUMBER,
				'step'      => 1,
				'default'   => 0,
				'selectors' => [
	...
	public function extends_column_params( $widget, $args ) {
		...
		$widget->start_controls_section( 'extened_header', [
			'label' => esc_html__( 'MPT Column Options', 'gpt-core' ),
			'tab'   => Controls_Manager::TAB_LAYOUT,
		] );

		$widget->add_control( 'apply_sticky_column', [
			'label'        => esc_html__( 'Enable Sticky?', 'gpt-core' ),
			'type'         => Controls_Manager::SWITCHER,
			'label_on'     => esc_html__( 'On', 'gpt-core' ),
			'label_off'    => esc_html__( 'Off', 'gpt-core' ),
